...
RayTracer::RayTracer(int32_t w, int32_t h, int _max_depth, int _samplesPerPixel)
    :m_Render(w, h), max_depth(_max_depth), samplesPerPixel(_samplesPerPixel), m_Depth(w, h) {}

// Computes the product of two square matrices in parallel.
void RayTracer::RenderParallel() {
...
// Measures the number of bounces of rays cast from the camera into the scene
void RayTracer::RenderDepth() {
    std::cout << "Starting Render of Depth.\n";
    ...
    parallel_for(size_t(0), width, [&](size_t x) {
        ...
        for (int y = 0; y < m_Render.height; y++) {
            ...
            for (int i = 0; i < samplesPerPixel; i++) {
                ...
                ray.Normalize();
                ray = m_Scene.camera.transform.ToWorldSpace(ray);
                depthTrace += TraceDepth(ray, 0);
            }
            int depthIntensity = ((float) depthTrace / (max_depth + 1) / samplesPerPixel) * 255;
            if (depthIntensity > maxDepthIntensity)
                maxDepthIntensity = depthIntensity;
            m_Depth.SetPixel(x, y, olc::Pixel(depthIntensity, depthIntensity, depthIntensity));
        }
    });

    // Renormalize the image, otherwise it will be dark.
    parallel_for(size_t(0), width, [&](size_t x) {
        for (int y = 0; y < m_Render.height; y++) {
            auto pixel = m_Depth.GetPixel(x, y);
            int newValue = ((float)pixel.r / maxDepthIntensity) * 255;
            m_Depth.SetPixel(x, y, olc::Pixel(newValue, newValue, newValue));
        }
    });
...
// Counts the number of bounces of a ray cast into the scene.
int RayTracer::TraceDepth(const Ray& ray, int depth) {
    Intersection i = m_Scene.Intersect(ray, 0.001f, INFINITY);
